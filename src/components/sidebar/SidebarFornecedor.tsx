// src/components/sidebar_fornecedor/SidebarFornecedor.tsx
    ...
    return (
        ...
        >
            ...
            <nav className={`flex flex-col gap-4 text-lg ${oculto ? 'items-center' : ''}`}>
                <Link to="/perfilfornecedor" className="flex items-center gap-3 px-2 py-2 rounded hover:bg-white/20 transition-colors">
                    <House size={24} />
                    <span className={oculto ? 'hidden' : ''}>Painel</span>
                </Link>
                <Link to="/categorias/gerenciar" className="flex items-center gap-3 px-2 py-2 rounded hover:bg-white/20 transition-colors">
                    <Tag size={24} />
                    <span className={oculto ? 'hidden' : ''}>Categorias</span>
                </Link>
                <Link to="/produtos/gerenciar" className="flex items-center gap-3 px-2 py-2 rounded hover:bg-white/20 transition-colors">
                    <Package size={24} />
                    <span className={oculto ? 'hidden' : ''}>Produtos</span>
                </Link>
            </nav>

            {/* Logo Fixo no Final */}
            <div className="mt-auto text-center p-4">
                <span className={`font-extrabold text-2xl tracking-wide transition-opacity duration-300 ${oculto ? 'opacity-0' : 'opacity-100'}`}>
                    DEVLIVERY
                </span>
            </div>
        </aside>
